...
if __name__ == '__main__':
    ...
    train_data, test_data, train_labels, test_labels = Prepare.split(data, labels, 0.2)
    train_data = Prepare.expand_dims(train_data)
    test_data = Prepare.expand_dims(test_data)

    model = Train.build_model((data.shape[1], data.shape[2], 1), output_len=Labels.from_char('z') + 1)

    model = Train.train(train_data, train_labels, model)

    loss, accuracy = Train.evaluate(model, train_data, train_labels)
